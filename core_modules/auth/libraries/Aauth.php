...
class Aauth {
	 ...
	 */
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL,$frontlogin = false) {
		...
		if ($this->config_vars['ddos_protection'] && ! $this->update_login_attempts()) {
			flashMsg('error', $this->CI->lang->line('aauth_error_login_attempts_exceeded'));
		...
		if($this->config_vars['ddos_protection'] && $this->config_vars['recaptcha_active'] && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']){
			...
			if( ! $resp->success){
				flashMsg('error', $this->CI->lang->line('aauth_error_recaptcha_not_correct'));
		...
		if($frontlogin){
		    if( !valid_email($identifier) OR strlen($pass) < $this->config_vars['min'] OR strlen($pass) > $this->config_vars['max'] )
    			{
    				flashMsg('error', $this->CI->lang->line('aauth_error_login_failed_email'));
		...
		if ($query->num_rows() > 0) {
			flashMsg('error', $this->CI->lang->line('aauth_error_account_not_verified'));
		...
		if($query->num_rows() == 0){
			flashMsg('error', $this->CI->lang->line('aauth_error_no_user'));
		...
		if($this->config_vars['totp_active'] == TRUE AND $this->config_vars['totp_only_on_ip_change'] == FALSE AND $this->config_vars['totp_two_step_login_active'] == FALSE){
			...
			if ($query->num_rows() > 0 AND !$totp_code) {
				flashMsg('error', $this->CI->lang->line('aauth_error_totp_code_required'));
				...
				if(!empty($totp_secret)){
					...
					if (!$checkResult) {
						flashMsg('error', $this->CI->lang->line('aauth_error_totp_code_invalid'));
	 ...
	 */
	public function control( $perm_par = FALSE, $redirect = TRUE ){
		if($this->CI->session->userdata('totp_required')){
			flashMsg('error', $this->CI->lang->line('aauth_error_totp_verification_required'));
		...
		// if($perm_id == '1003'){
		// 	var_dump($this->is_group_allowed($perm_id));
		// 	var_dump(( ! $this->is_allowed($perm_id) || ! $this->is_group_allowed($perm_id) ));
		// }
		$this->update_activity();
		if($perm_par == FALSE){
			...
			if($this->is_loggedin()){
				...
				flashMsg('warning',$this->CI->lang->line('aauth_error_no_access'));
		...
		}else if ( ! $this->is_allowed($perm_id) && ! $this->is_group_allowed($perm_id) ){
			if ( $redirect ) {
	...
	public function remind_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$data['year'] = date('Y');
			$data['confirmation_url'] = site_url($this->config_vars['reset_password_link'] . $ver_code);
			$message_body = $this->CI->parser->parse('auth/email_templates/forgot_password', $data, TRUE);	
	...
	public function reset_password($ver_code){
		...
		if( $query->num_rows() > 0 ){
			...
			$data['email'] = $row->email;
			$data['year'] = date('Y');
			$message_body = $this->CI->parser->parse('auth/email_templates/reset_password', $data, TRUE);	
	 ...
	 */
	public function create_user($email, $pass, $username = FALSE, $fullname = false) {
		$valid = TRUE;
		if($this->config_vars['login_with_name'] == TRUE){
			if (empty($username)){
		...
		if ( $this->aauth_db->insert($this->config_vars['users'], $data )){
			...
			// if verification activated, 2nd CONDITION UPDATED
	 ...
	 * @return bool Update fails/succeeds
	 */
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE, $fullname = false) {
	 ...
	 * @return object User information
	 */
	public function check_old_password($user_id = FALSE, $pass = FALSE) {

		if ($user_id == FALSE || $pass == FALSE)
			return FALSE;
	 ...
	 * Activates user account based on verification code
	 * @param int $user_id User id to activate
	 * @param string $toggle LOCK|UNLOCK
	 * @return bool Activation fails/succeeds
	 */
	public function toggle_user($user_id, $toggle){
	...
	public function send_verification($user_id){
		...
		if ($query->num_rows() > 0){
			...
			$pass_length = ($this->config_vars['min']&1 ? $this->config_vars['min']+1 : $this->config_vars['min']);
			$password = random_string('alnum', $pass_length);
			...
			$data['email'] = $row->email;
			$data['year'] = date('Y');
			$data['confirmation_url'] = site_url($this->config_vars['verification_link'] . $user_id .'/'. $verification_code); 
			
			$message_body = $this->CI->parser->parse('auth/email_templates/user_registration', $data, TRUE);

			
			$password = $this->hash_password($password, $user_id);

			$this->aauth_db->where('id', $user_id);
			$this->aauth_db->update($this->config_vars['users'], array('pass' => $password, 'verification_code' => $verification_code));
	 ...
	 */
	public function create_group($id, $group_name, $definition = '') {
		...
		if ($query->num_rows() < 1) {
			...
			$res = $this->aauth_db->insert($this->config_vars['groups'], $data);
			return $res;
		}
