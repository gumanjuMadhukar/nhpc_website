...
class AdminStaffs extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->staff_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->staff_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->staff_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
			$this->db->or_like('mst_staffs.name', $params['search']['value']);
			$this->db->or_like('mst_staffs.designation', $params['search']['value']);
			$this->db->group_end();
		}
	...
	public function save()
	{
        ...
        {
            $success=$this->staff_model->insert($data);
        }
        else
        {
            $success=$this->staff_model->update($data['id'],$data);
        }
   ...
   public function delete_json()
    ...
    function do_upload(){
        ...
        if($this->upload->do_upload()){
            ...
            $config['new_image'] = './uploads/staffs/thumbs/'.$image;
			$config['source_image'] = './uploads/staffs/'.$image;
