...
class AdminBanners extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->banner_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->banner_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->banner_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
$this->db->or_like('mst_banners.name', $params['search']['value']);
	...
	public function save()
	{
        ...
        {
            $success=$this->banner_model->insert($data);
        }
        else
        {
            $success=$this->banner_model->update($data['id'],$data);
        }
   ...
   private function _get_posted_data()
   {
    ...
    function do_upload(){
        ...
        if($this->upload->do_upload()){
            ...
            $config['new_image'] = './uploads/banners/thumbs/'.$image;
			$config['source_image'] = './uploads/banners/'.$image;
