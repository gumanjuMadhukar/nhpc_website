...
class AdminSyllabuses extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->syllabus_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->syllabus_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->syllabus_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
$this->db->or_like('mst_syllabus.name', $params['search']['value']);
	...
	public function save()
	{
        ...
        {
            $success=$this->syllabus_model->insert($data);
        }
        else
        {
            $success=$this->syllabus_model->update($data['id'],$data);
        }
   ...
   public function delete_json()
    {
        ...
        $this->syllabus_model->delete($id[0]);
    ...
    public function upload_pdf()
   	{
		...
		$config['upload_path'] = "uploads/syllabus_pdf";
	...
	function upload_delete()
	{
		...
		@unlink('uploads/syllabus_pdf/' . $filename);
	}
}