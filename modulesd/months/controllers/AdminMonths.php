...
class AdminMonths extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->month_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->month_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->month_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
$this->db->or_like('mst_month.name', $params['search']['value']);
$this->db->or_where('mst_month.rank', $params['search']['value']);
	...
	public function save()
	{
        ...
        {
            $success=$this->month_model->insert($data);
        }
        else
        {
            $success=$this->month_model->update($data['id'],$data);
        }
