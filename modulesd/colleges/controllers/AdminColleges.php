...
class AdminColleges extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->college_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->college_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->college_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
$this->db->or_like('mst_colleges.name', $params['search']['value']);
// $this->db->or_like('mst_colleges.code', $params['search']['value']);
// $this->db->or_like('mst_colleges.address', $params['search']['value']);
// $this->db->or_like('mst_colleges.link', $params['search']['value']);
	...
	public function save()
	{
        ...
        {
            $success=$this->college_model->insert($data);
        }
        else
        {
            $success=$this->college_model->update($data['id'],$data);
        }
   ...
   public function delete_json()
    {
        ...
        $this->college_model->delete($id[0]);
