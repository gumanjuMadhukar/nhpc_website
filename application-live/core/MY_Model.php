...
class MY_Model extends CI_Model
{
     ...
     */
    protected $protected_attributes = array('created_by', 'updated_by', 'deleted_by', 'created_at', 'updated_at', 'deleted_at');
    ...
    protected function _initialize_schema()
    {
        ...
        /* add following lines of code by <AUTHOR_NAME> Starts*/
        $this->subscribe('after_create', 'activity_log_insert');
        $this->subscribe('before_update', 'audit_log_update');
        // $this->subscribe('after_create', 'audit_log_update');
        /* add following lines of code by <AUTHOR_NAME> Ends */
    ...
    {
        $watch = array('find_by', 'find_all_by', 'find_field_by', 'findBy', 'find_allBy', 'findFieldBy');
    ...
    public static function underscore_from_camel_case($str) {
        ...
        $str[0] = strtolower($str[0]);
        // $func = create_function('$c', 'return "_" . strtolower($c[1]);');
        $func = function($c) use ( $str) {
            return  $str ."_" . strtolower($c[1]);
        };
        return preg_replace_callback('/([A-Z])/', $func, $str);
    }
    ...
    public function count_by()
    {
        ...
        // foreach($this->_joins as $value){
        //     $fields=$fields . ','.$this->_JOINS[$value]['select'];
        // }

        foreach($this->_joins as $key):
                    $this->_database->join($this->_JOINS[$key]['table']. ' ' .$this->_JOINS[$key]['alias'],$this->_JOINS[$key]['join_field'],$this->_JOINS[$key]['join_type']);
        endforeach;  
    ...
    protected function apply_soft_delete_filter()
    {
        if ($this->soft_delete && $this->_temporary_with_deleted !== TRUE) {
            ...
            {
                $where = "$this->table.`{$this->deleted_at_key}` <= NOW()";
            }
            else
            {
                // $where = sprintf('(%1$s > NOW() OR %1$s IS NULL OR %1$s = \'0000-00-00 00:00:00\')', $this->deleted_at_key);
                $where = sprintf('('.$this->_table.'.%1$s > NOW() OR '.$this->_table.'.%1$s IS NULL )', $this->deleted_at_key);
            }
    ...
    /* Activty Logs */
    public function activity_log_insert($id)
    {
        ...
        $this->_database->insert('project_activity_logs', $activity_log);

        return $row;
    }

    /* Audit Logs */
    public function audit_log_update($row)
    {
        ...
        if (count($audit_log) > 0) {
            $this->_database->insert_batch('project_audit_logs', $audit_log);
            $this->_database->insert('project_activity_logs', $activity_log);
        }
    ...
    public function remove_protected_attributes($attribute = null) {
        if ($attribute != null) {
            if(($key = array_search($attribute, $this->protected_attributes)) !== false) {
                unset($this->protected_attributes[$key]);
            }
        }
    }

    public function exec_cli() {
        $this->unsubscribe('before_create', 'created_by');
        $this->unsubscribe('before_create', 'created_at');
        $this->unsubscribe('before_create', 'updated_by');
        $this->unsubscribe('before_create', 'updated_at');

        $this->unsubscribe('before_update', 'updated_by');
        $this->unsubscribe('before_update', 'updated_at');

        $this->remove_protected_attributes('created_by');
        $this->remove_protected_attributes('updated_by');
        $this->remove_protected_attributes('created_at');
        $this->remove_protected_attributes('updated_at');
    }

    public function disable_soft_delete() {
