...
class AdminCodes extends Admin_Controller
{
	var $table_name = '';
	var $prefix;
	...
	public function generate()
	{
		...
		foreach($this->input->post('tables') as $table) {
			...
			$view_array = array(
							...
							'MODULE_URL'				=> plural(ucfirst($this->module_name)),
			...
			$data = array(
				'name' => humanize(plural($this->module_name)),
				'definition' => humanize(plural($this->module_name))	
			);

			$this->db->insert('aauth_permissions',$data);
...
\t\t\t},\r\n";
	...
	private function _generate_column_fields_backup($fields)	
	{
		$cols = '';
			...
			if (!in_array($row->name, $this->discard)) {
				/*if (!preg_match('/text/is', $row->type, $match) && !preg_match('/image/is', $row->name, $match)) {
					if (preg_match('/tinyint/', $row->type, $match)) {
						$cols = $cols . "(isset(\$params['search']['" . $row->name . "']))?\$this->db->or_where('" . $row->name . "', \$params['search']['" . $row->name . "']):'';\r\n";		
					} else if (preg_match('/int/', $row->type, $match)) {
						$cols = $cols . "(\$params['search']['" . $row->name . "'] != '')?\$this->db->or_where('" . $row->name . "', \$params['search']['" . $row->name . "']):'';\r\n";						
					} else if (!preg_match('/date/', $row->type, $match)) {
						$cols = $cols . "(\$params['search']['" . $row->name . "'] != '')?\$this->db->or_like('" . $row->name . "', \$params['search']['" . $row->name . "']):'';\r\n";
					}

				}*/
				if (!preg_match('/text/is', $row->type, $match) && !preg_match('/image/is', $row->name, $match)) {
					if (preg_match('/tinyint/', $row->type, $match)) {
						$cols = $cols . "\$this->db->or_where('" . $this->prefix . $this->table_name . '.' . $row->name . "', \$params['search']['value']);\r\n";		
					} else if (preg_match('/int/', $row->type, $match)) {
						$cols = $cols . "\$this->db->or_where('" . $this->prefix . $this->table_name . '.' . $row->name . "', \$params['search']['value']);\r\n";						
					} else if (!preg_match('/date/', $row->type, $match)) {
						$cols = $cols . "\$this->db->or_like('" . $this->prefix . $this->table_name . '.' . $row->name . "', \$params['search']['value']);\r\n";
					}
			...
			if (
				...
				(!$this->input->post('discard_grid') && !in_array($row->name, $this->discard)) ) {
					...
					if (strpos($row->name, 'status') !== FALSE ||strpos($row->name, 'active') !== FALSE) {
						$cols = $cols . "<th ><?php echo lang('" . $row->name . "')?></th>\r\n";
					} else {
						//if ($row->primary_key == '1') {
		...
		foreach($fields as $row) {
			if ($row->name != $primary_key && !in_array($row->name, $this->discard)) {
				...
				$mandatory   = ($row->name == 'name' || $row->name == 'rank') ? "<span class='mandatory'>*</span>" : '';
						...
						if ($type == 'datetime') {
							$class = "\r\n\t\t\t\t<input id='{$id}' class='form-control datetimepicker' name='{$name}'>";
						}elseif(preg_match('/text/',$row->type,$match))
						{
							$class = "\r\n\t\t\t\t<textarea name='{$name}' id='{$id}' class='form-control' ></textarea>";
						}elseif ($type == 'date') {
							$class = "\r\n\t\t\t\t<input id='{$id}' class='form-control datepicker' name='{$name}'>";
						} elseif (preg_match('/int/is', $type, $matches)) {
							$class = "\r\n\t\t\t\t<input id='{$id}' type='number' class='form-control' name='{$name}'>";
						}elseif(preg_match('/tinyint/',$row->type,$match))
						{
							$class=	"\r\n\t\t\t\t<input type='radio' value='1' name='{$name}' id='{$id}_1' /><label for='{$id}_1'><?php echo lang('general_yes')?></label> <input type='radio' value='0' name='{$name}' id='{$id}_0' /><label for='{$id}_0'><?php echo lang('general_no')?></label>";				
						}
						// $class .=  "</div>"

						$cols .= "\r\n\t\t\t\t<div class='form-group'>";
						$cols .= "\r\n\t\t\t\t<label for='{$id}'><?php echo lang('{$name}')?>{$mandatory}</label>";
						$cols .= $class;
						$cols .= "\r\n\t\t\t\t</div>";
	...
	private function _generate_form_fields_backup($fields, $primary_key)
	{
		$cols = '';
		$module = plural($this->module_name);
