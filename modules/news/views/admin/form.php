    ...
    <div class="card">
        ...
        <div class="card-body">
            <?php echo form_open('', array('id' =>'form-news')); ?>
                    <input type = "hidden" name = "id" id = "id" value="<?php echo $row->id?$row->id:''?>" />
                <div class='form-group'>
                <label for='news_name'><?php echo lang('name')?><span class='mandatory'>*</span></label>
                <input id='news_name' type='text' class='form-control' name='name' value="<?php echo $row->name?$row->name:''?>" >
                </div>
                <div class='form-group'>
                <label for='image'><?php echo lang('image')?></label>
                    <div id="image_upload_name" style="display:none"></div>
                    <input name="image" id="image" class='text_input' value="<?php $row->image?$row->image:''?>" style="display:none"/>
                    <input type="file" id="image_upload" name="userfile" style="display:block"/>
                </div>
                <div class='form-group'>
                <label for='description'><?php echo lang('des€cription')?></label>
                <textarea name='description' id='description' class='form-control' ><?php echo $row->date?$row->description:''?></textarea>
                </div>
                <div class='form-group'>
                <label for='date'><?php echo lang('date')?></label>
                <input id='date' class='form-control datepicker' name='date' value="<?php echo $row->date?$row->date:''?>">
                </div>
                <div class='form-group'>
                <input type="checkbox" id='is_featured' name='is_featured' <?php echo $row->is_featured?'checked':''?>> <label for='date'><?php echo lang('is_featured')?></label>
                </div>
