...
class AdminSubject_committee_types extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->subject_committee_type_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->subject_committee_type_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->subject_committee_type_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
...
$this->db->or_like('mst_subject_committee_type.name', $params['search']['value']);
$this->db->or_where('mst_subject_committee_type.rank', $params['search']['value']);
	...
	public function save()
	{
        ...
        {
            $success=$this->subject_committee_type_model->insert($data);
        }
        else
        {
            $success=$this->subject_committee_type_model->update($data['id'],$data);
        }
   ...
   public function delete_json()
    {
        ...
        $this->subject_committee_type_model->delete($id[0]);
