...
class AdminExpertises extends Admin_Controller
{
	...
	public function json()
	{
		...
		if($params['program']){
			$where["subject LIKE '%".$params['program']."%'"] = NULL;
		}
		if($params['level']){
			$where["level LIKE '%".$params['level']."%'"] = NULL;
		}
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
			$this->db->or_like('view_experts.full_name', $params['search']['value']);
			$this->db->or_like('view_experts.level', $params['search']['value']);
			$this->db->or_like('view_experts.subject', $params['search']['value']);
			$this->db->or_like('view_experts.mobile', $params['search']['value']);
			$this->db->or_like('view_experts.email', $params['search']['value']);
			$this->db->or_like('view_experts.phone', $params['search']['value']);
			// $this->db->or_where('view_experts.temp_province_id', $params['search']['value']);
			// $this->db->or_where('view_experts.temp_district_id', $params['search']['value']);
			// $this->db->or_where('view_experts.temp_city_place_id', $params['search']['value']);
			// $this->db->or_where('view_experts.temp_ward', $params['search']['value']);
			// $this->db->or_like('view_experts.temp_address', $params['search']['value']);
			// $this->db->or_like('view_experts.level', $params['search']['value']);
			// $this->db->or_like('view_experts.qualification', $params['search']['value']);
			// $this->db->or_like('view_experts.registration_no', $params['search']['value']);
			// $this->db->or_like('view_experts.subject', $params['search']['value']);
			// $this->db->or_like('view_experts.experiance', $params['search']['value']);
			// $this->db->or_like('view_experts.doc_cv', $params['search']['value']);
			// $this->db->or_like('view_experts.doc_certificate', $params['search']['value']);
	...
	public function save()
	{
   	...
   	public function export_excel()
   	{
    	...
    	$output = "
	    	<table border='1'>
	    		<tr>
	    			<td colspan=10 style='text-align:center'>LISTS of  EXPERTS </td>
	    		</tr>
	    		<tr>
	    			<td></td>
