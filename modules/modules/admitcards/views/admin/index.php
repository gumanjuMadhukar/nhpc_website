...
<section class="content">
    ...
    <div class="card">
        <div class="card-header">
			...
			<a href="<?php echo site_url('admin/Admitcards/generate_remaining') ?>" class="btn btn-sm btn-primary btn-outline btn-round"  title="Generate Remaining Card">
			<i class="icon wb-plus" aria-hidden="true"></i>
        	<span class="hidden-sm-down">Generate Remaining Card</span>
			</a>
			...
			<!-- result button -->
			<button class="btn btn-sm btn-primary btn-outline btn-round"  title="Result" id="result_upload">
			<i class="icon wb-plus" aria-hidden="true"></i>
        	<span class="hidden-sm-down">Upload Result</span>
			</button>
		...
		dataTable = $('#Admitcard-table').DataTable({
			...
			'ajax' : { url: "<?php  echo site_url('admin/Admitcards/json'); ?>",type: 'POST' },
				...
				{ data: function(data,b,c,table) { 
						...
						buttons += "<a href='<?php echo site_url();?>/"+url+"' class='btn btn-sm btn-success btn-outline'  title='Edit' target='_blank'><i class='fas fa-folder nav-icon'></i></a>&nbsp;&nbsp"; 
