...
class AdminPublications extends Admin_Controller
{
	...
	public function json()
	{
		...
		$total=$this->publication_model->find_count();

		/*Count for filters*/
		$this->_get_search_params($params);	
		$filter_total=$this->publication_model->find_count($where);

		/*Filters*/
		$this->_get_search_params($params);	
		$rows=$this->publication_model->find_all($where,'*',null,$this->input->post('start'),$this->input->post('length'));
	...
	public function _get_search_params($params)
	{
		...
		if($params['search']['value'] != ''){
			$this->db->group_start();
$this->db->or_like('mst_publications.name', $params['search']['value']);
	...
	public function save()
	{
        ...
        {
            $success=$this->publication_model->insert($data);
        }
        else
        {
            $success=$this->publication_model->update($data['id'],$data);
        }
   ...
   private function _get_posted_data()
   {
     ...
     public function delete_json()
    {
        ...
        $this->publication_model->delete($id[0]);
